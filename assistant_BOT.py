        ...
        while True:
            ...
            if "home status" in userresponcestring:
                ...
                if "yes" in userresponcestring:                    
                    ...
                    text_msg = 'The current kennel state: electricity average consumption is '+ E_report +' kiloWatt per hour and operated under normal condition, Sensitivity average consumption is '+ W_report +' cubic meters per hour and it is usial to current session'
            ...
            if "kennel temperature" in userresponcestring:
                # here is analitics request to manager
                icA('Data request..')
                temperature = da.read_IOT_data('data', 'DHT-1')[-1][2]
                textS='The kennel temperature is ' + temperature + ' celcius degrees , whould you like to turn on the air conditioner ?'
                icA(textS)
                ...
                if 'yes' in userresponcestring:
                    ...
                    icA('How many celcius degrees would you like?')
                    if path.exists("how_many_celcius.wav"):
                        pl.play("how_many_celcius.wav")
                    else:    
                        ts.save2file(ts.tts_request('How many celcius degrees would you like?'),ttsfile)
                        pl.play(ttsfile)
                    time.sleep(sys_delay)
